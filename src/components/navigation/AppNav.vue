...
<template>
  <header
    class="fixed top-0 left-0 right-0 z-30 py-4 transition-all duration-200 bg-zinc-50/80 dark:bg-zinc-950/80 backdrop-blur-md"
    :class="headerClasses"
  >
    <div class="max-w-[1200px] mx-auto px-6 flex items-center justify-between">
      <a href="#" class="flex items-center font-heading text-xl cursor-pointer">
      ...
      <nav class="hidden md:flex items-center gap-8" aria-label="Main navigation">
        <a
          ...
          :key="item.key"
          :href="item.href"
          class="text-zinc-700 dark:text-zinc-300 text-sm font-medium hover:text-musgo-600 dark:hover:text-musgo-400 transition-colors duration-150 cursor-pointer"
        >
          {{ t(`nav.${item.key}`) }}
        </a>
      </nav>
      ...
      <div class="flex items-center gap-1">
        ...
        >
          <span class="flex flex-col gap-1.5">
            <span
              class="block w-6 h-0.5 bg-current transition-all duration-150"
