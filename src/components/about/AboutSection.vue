...
<template>
  ...
  >
    <template #default="{ isVisible }">
      <div
        class="grid md:grid-cols-2 gap-12 md:gap-16 items-center mb-16 transition-[opacity,transform] duration-500 ease-out"
        :class="isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'"
        :style="{ transitionDelay: '0.15s' }"
      >
        <BaseTypography as="large-body" variant="muted" class="leading-relaxed">
          {{ t('about.description') }}
        </BaseTypography>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
        ...
        >
          <div class="flex flex-col items-center text-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-musgo-100 dark:bg-musgo-900 flex items-center justify-center">
